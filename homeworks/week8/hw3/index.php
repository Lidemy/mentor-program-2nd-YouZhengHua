			...
			<div class="col-lg-4 mb-3">
				<div class="item card" value="<?php echo $resultRow['id']; ?>">
					...
					<div class="card-body">
						...
						<p class="card-text"><?php echo $resultRow['contnet']; ?></p>
			...
			<div class="orderList row mb-3" style="display: none">
				<div class="card mx-auto col-lg-10 p-0">
					<table class="table table-responsive-lg mb-0 table-hover">
						...
						<tbody>
							<?php 
								$stmt = $conn->prepare("SELECT a.id, a.order_date, a.order_price, CASE WHEN b.nickname IS NULL OR b.nickname = '' THEN b.username ELSE b.nickname END AS nickname FROM marin_order a JOIN marin_users b ON b.id = a.order_user ORDER BY id");
								$stmt->execute();
								$result = $stmt->get_result();
								$stmt->close();
								$index = 1;
